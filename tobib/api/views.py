...
# Get a specific Chat room according to the ID
@swagger_auto_schema(
    responses={"200": ChatBotRoomSerializer, "404": "Ressource not found"}, method="get"
)
...
@swagger_auto_schema(
    method="post",
    request_body=CreateChatBotRoomSerializer,
    responses={"201": ChatBotRoomSerializer},
)
@swagger_auto_schema(method="get", responses={"200": ChatBotRoomSerializer})
@api_view(["GET", "POST"])
@permission_classes([IsAuthenticated])
def chat_bot_rooms_list(request):
    ...
    if request.method == "GET":
        ...
        serializer = ChatBotRoomSerializer(data={**request.data, "user": user.id})
        if serializer.is_valid(raise_exception=True):
            serializer.save()
    ...
    if request.method == "GET":
        ...
        if serializer.is_valid():
            ...
            if chat_bot_room.number_of_messages == 0:
                ...
                answer_message = ChatBotRoomMessage(
                    text_content=answer,
                    is_bot=True,
                    chat_room=chat_bot_room,
                    answer_number=1,
                )
                answer_message.save()
